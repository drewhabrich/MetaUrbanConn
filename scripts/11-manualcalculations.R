## HEADER---------------------------
## Script name: 11-manualcalculations
##
...
pacman::p_load(tidyverse, easystats, ggpubr, corrplot, vegan)
...
eckert2017 <- read_csv("./raw_figextraction/suppls/Eckert2017specrich.csv") %>% 
  mutate(S = vegan::specnumber(eckert_sites[,2:22]),
         H = vegan::diversity(eckert_sites[,2:22], index = "shannon"),
         D = vegan::diversity(eckert_sites[,2:22], index = "invsimpson"), #1/D 
         evenness = H/log(S)
  )
...
st_2013 <- st_2013 %>% mutate(simp_rindex = 1/simp_dom,  #convert to simpson reciprocal index
                              enn_dist = 1/conn) #convert to mean nearest neighbour distance
st_2013 %>% select(7:13) %>% correlation()

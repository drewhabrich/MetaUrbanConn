...
final_tiab <- read_csv("./data/final_tiab_screening-04-1.csv")
...
# di_yes %>%
#   filter(!is.na(doi)) %>%
#   mutate(doilabel = paste0("https://doi.org/", doi)) %>% 
#   mutate(pdflabel = paste0(label,".pdf")) %>% View
#   get_and_save(data = ., links = "doilabel", 
#                save_names = "pdflabel", dir = "./pdfs/yes",
#                print_every = 5, dupe_strategy = "ignore")
...
## 3.3 How many records are available open-access? ----
# Lets check to see availability of open-access pdfs (using oaDOI and unpaywall)
# Write the dataset to .csv to investigate the dataframe later. We *should* be able to extract open-access locations for the pdfs!
...
# 4. Download PDFs --------------------------------------------------------
## 4.1 using 'heapsofpapers' to download from URL ----
bib_url %>%
  filter(!is.na(url_for_pdf)) %>%
  filter(INCLUDE == "YES") %>% count()
### check how many have already been downloaded
